...
<!-- Mobile Menu Button -->
<button id="mobile-menu-button" class="lg:hidden fixed top-4 left-4 z-50 bg-[#3A5F6B] text-white p-2 rounded-lg shadow-lg">
...
<div id="sidebar" class="w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-white flex flex-col fixed h-full shadow-2xl z-40 transform -translate-x-full lg:translate-x-0 transition-all duration-300 ease-in-out border-r border-slate-700">
    <!-- Logo Section -->
    <div class="p-6 lg:p-8 border-b border-slate-700 bg-gradient-to-r from-slate-800 to-slate-700">
        <div class="flex items-center space-x-4">
            ...
            <button id="mobile-close-button" class="lg:hidden p-2 rounded-lg hover:bg-slate-700 transition-colors">
    ...
    <nav class="flex-1 flex flex-col px-6 py-8 space-y-2 overflow-y-auto">
        @if($role === 'admin')
            ...
            <div class="mb-6">
                ...
                <!-- Dashboard -->
                <a href="/dashboard" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'dashboard') || $currentRoute === 'admin.dashboard' ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg' : '' }}">
                ...
                <!-- User Management -->
                <a href="/users" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-green-600 hover:to-emerald-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'users') ? 'bg-gradient-to-r from-green-600 to-emerald-600 text-white shadow-lg' : '' }}">
                ...
                <!-- Events -->
                <a href="/events" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-purple-600 hover:to-pink-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'events') ? 'bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg' : '' }}">
                ...
                <!-- Applicants -->
                <a href="/students" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-indigo-600 hover:to-blue-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'students') ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-lg' : '' }}">
                ...
                <!-- Volunteers -->
                <a href="{{ route('admin.volunteers.index') }}" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-orange-600 hover:to-red-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'volunteers') || str_contains($currentRoute, 'volunteer-applications') ? 'bg-gradient-to-r from-orange-600 to-red-600 text-white shadow-lg' : '' }}">
                ...
                <!-- Jobs -->
                <a href="{{ route('admin.jobs.index') }}" class="group flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-teal-600 hover:to-cyan-600 transition-all duration-300 rounded-xl mb-1 {{ str_contains($currentRoute, 'jobs') ? 'bg-gradient-to-r from-teal-600 to-cyan-600 text-white shadow-lg' : '' }}">
        ...
        @elseif($role === 'volunteer')
            ...
            <a href="{{ route('volunteer.jobs') }}" class="sidebar-link flex items-center py-3 px-4 rounded-lg transition-all duration-300 ease-in-out hover:bg-[#2C5F6E] hover:shadow-md transform hover:scale-105 {{ str_contains($currentRoute, 'jobs') ? 'bg-[#2C5F6E] shadow-md' : '' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2-2v2m8 0V6a2 2 0 012 2v6M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M8 6v10a2 2 0 002 2h4a2 2 0 002-2V6"/>
                </svg>
    ...
    <div class="mt-auto px-6 py-6 border-t border-slate-700 bg-gradient-to-r from-slate-800 to-slate-700">
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="group w-full flex items-center py-3 px-4 text-slate-300 hover:text-white hover:bg-gradient-to-r hover:from-red-600 hover:to-red-700 transition-all duration-300 rounded-xl">
